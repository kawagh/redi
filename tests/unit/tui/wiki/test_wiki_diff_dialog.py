...
from redi.tui.wiki.version_dialog import select_version
...
class TestDiffDialog:
    ...
    def test_from_is_previous_and_to_is_latest_when_viewing_latest(self):
        ...
        assert open_diff_dialog(state) is True

        dialog = state.wiki_tab.diff_dialog
        assert dialog.show is True
        assert dialog.focus == "from"
        assert dialog.versions == [3, 2, 1]
        assert dialog.versions[dialog.cursors["from"]] == 2
        assert dialog.versions[dialog.cursors["to"]] == 3
    ...
    def test_from_is_viewing_version(self):
        ...
        open_diff_dialog(state)

        dialog = state.wiki_tab.diff_dialog
        assert dialog.versions[dialog.cursors["from"]] == 1
        assert dialog.versions[dialog.cursors["to"]] == 3
    ...
    def test_reopen_keeps_applied_pair(self):
        ...
        dialog = state.wiki_tab.diff_dialog
        assert dialog.versions[dialog.cursors["from"]] == 3
    ...
    def test_single_version_flashes(self):
        ...
        assert state.wiki_tab.diff_dialog.show is False
...
class TestDiffDialogCursor:
    """jk / gg / G は focus のある列だけを動かし、端で止まる"""

    def _dialog(self):
        state = _state([_page("Home", version=3)])
        open_diff_dialog(state)
        return state.wiki_tab.diff_dialog
    ...
    def test_moves_focused_column_only(self):
        ...
        move_cursor(dialog, 1)

        assert dialog.cursors == {"from": 1, "to": 1}

    def test_stops_at_edges(self):
        """先頭より上、末尾より下へは動かない"""
        dialog = self._dialog()

        move_cursor(dialog, -5)
        assert dialog.cursors["from"] == 0
        move_cursor(dialog, 5)
        assert dialog.cursors["from"] == 2

    def test_jump_to_top_and_bottom(self):
        """gg / G で先頭・末尾へ飛ぶ"""
        dialog = self._dialog()

        move_cursor_to_end(dialog, top=False)
        assert dialog.cursors["from"] == 2
        move_cursor_to_end(dialog, top=True)
        assert dialog.cursors["from"] == 0
